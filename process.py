...
def output_array(channel: Channel, dst, suffix, format, ticks_per_whole_note, tempo):
    ...
    if format == 'integer':
        ...
        result += ' '.join([MusicMapping.int_to_name[p]
                           for p in channel.pitch])
    result += '\n'
    result += ' '.join([str(d / ticks_per_whole_note)
                       for d in channel.duration])
    result += '\n'
    if tempo != None:
        result += str(tempo)
...
def midi_to_array(input, output, format, no_rest):
    mid = MidiFile(input)
    ...
    for i, track in enumerate(mid.tracks):
        ...
        for j, channel in enumerate(channels):
            ...
            if len(channel) > 1:
                file_names.append(output_array(
                    channel, output, f'-track-{i}-channel{j}', format, ticks_per_whole_note, tempo))
    return file_names
...
def array_to_midi(input, output, format, bpm):
    ...
    with open(input, 'r') as fin:
        ...
        notes = process_array_input(
            pitches, duration, format, ticks_per_whole_note)
...
def convert(input, output, format, bpm, no_rest):
    if input.endswith('.mid') or input.endswith('.midi') or input.endswith('.MID') or input.endswith('.MIDI'):
        file_names = midi_to_array(input, output, format, no_rest)
